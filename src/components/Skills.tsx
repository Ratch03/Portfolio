              ...
              <div key={group.category} className="skill-row">
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.6rem' }}>
                  {group.items.map(item => (
                    <span key={item} style={{ fontSize: '0.82rem', padding: '0.35rem 0.9rem', border: '0.5px solid #333', color: '#fff', borderRadius: '2px', letterSpacing: '0.02em', fontWeight: 700 }}>{item}</span>
                  ))}
                </div>
              </div>
            ))}
